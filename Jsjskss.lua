-- ============================================================================
-- 👑 BROSA SYSTEM v5.2 — PRIVATE ELITE MONOLITH SCRIPT HUB [ЧАСТЬ 1 ИЗ 3]
-- 🛠️ Среда выполнения: Delta Executor / Спецификация движка: Luau (Roblox API)
-- 🎯 Оптимизировано под режим: Fling Things and People (FTAP) + Магнитный FOV Aim
-- ============================================================================
...
-- [РЕЕСТР КОНФИГУРАЦИИ] Расширенная таблица всех функций, слайдеров и внутренних данных хаба
_G.BrosaHub = {
	Flags = {
		-- Combat & Fling
	...
	Options = {
		...
		FlySpeed = 50,
		FlingPower = 6500,
		AuraRadius = 100, -- Базовый радиус круга FOV для магнитного аима
		OrbitRadius = 7,
		OrbitSpeed = 12,
		ChatSpamDelay = 2.5,
		ThrowForce = 150,
...
if rawmetatable and make_writeable then
	...
	warn("[BROSA SYSTEM] Внимание: Исполнитель не поддерживает низкоуровневые хуки метатаблиц.")
end

-- [ДИЗАЙН-СИСТЕМА] Перенос палитры BROSA v4.0 в Roblox цвета
local Colors = {
...
local oldUI = CoreGui:FindFirstChild("BrosaSystemV4_UI")
if oldUI then oldUI:Destroy() end

-- [ИНТЕРФЕЙС] Создание корневого контейнера ScreenGui под защитой CoreGui
local MainGui = Instance.new("ScreenGui")
MainGui.Name = "BrosaSystemV4_UI"
MainGui.ResetOnSpawn = false
MainGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
if gethui then MainGui.Parent = gethui() else MainGui.Parent = CoreGui end
...
WindowContainer.Parent = MainGui
...
LoadingScreen.Parent = WindowContainer
...
LoaderTitle.Parent = LoadingScreen

local LoaderBarBg = Instance.new("Frame")
LoaderBarBg.Name = "LoaderBarBg"
...
local LoaderBarFillCorner = Instance.new("UICorner")
LoaderBarFillCorner.CornerRadius = UDim.new(0, 10)
LoaderBarFillCorner.Parent = LoaderBarFill

local LoaderBarGlow = Instance.new("ImageLabel")
LoaderBarGlow.Name = "LoaderBarGlow"
...
LoaderBarGlow.Parent = LoaderBarFill
...
MainPanel.Parent = WindowContainer
...
-- Кроссплатформенный Dragging Engine для стабильного перетаскивания меню на Delta
...
-- Временная шкала интерполяции лоадера
task.spawn(function()
	local tweenInfo1 = TweenInfo.new(1.1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out)
	local tween1 = TweenService:Create(LoaderBarFill, tweenInfo1, {Size = UDim2.new(0.5, 0, 1, 0)})
	tween1:Play() tween1.Completed:Wait() task.wait(0.2)

	local tweenInfo2 = TweenInfo.new(0.8, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out)
	local tween2 = TweenService:Create(LoaderBarFill, tweenInfo2, {Size = UDim2.new(0.85, 0, 1, 0)})
	tween2:Play() tween2.Completed:Wait() task.wait(0.1)

	local tweenInfo3 = TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.In)
	local tween3 = TweenService:Create(LoaderBarFill, tweenInfo3, {Size = UDim2.new(1, 0, 1, 0)})
	tween3:Play() tween3.Completed:Wait() task.wait(0.2)

	local screenHideInfo = TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local hideTween = TweenService:Create(LoadingScreen, screenHideInfo, {
		BackgroundTransparency = 1, Size = UDim2.new(0, 882, 0, 588), Position = UDim2.new(0, -21, 0, -14)
	})
...
hideTween:Play() hideTween.Completed:Wait() LoadingScreen:Destroy()
...
local panelShowInfo = TweenInfo.new(0.5, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out)
TweenService:Create(MainPanel, panelShowInfo, {Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 0}):Play()
end)
-- Сайдбар навигации
local Sidebar = Instance.new("Frame")
Sidebar.Name = "Sidebar" Sidebar.Size = UDim2.new(0, 230, 1, 0) Sidebar.BackgroundColor3 = Colors.BgSidebar Sidebar.BorderSizePixel = 0 Sidebar.Parent = MainPanel
local SidebarStroke = Instance.new("Frame")
SidebarStroke.Name = "SidebarStroke" SidebarStroke.Size = UDim2.new(0, 1, 1, 0) SidebarStroke.Position = UDim2.new(1, -1, 0, 0) SidebarStroke.BackgroundColor3 = Colors.Border SidebarStroke.BorderSizePixel = 0 SidebarStroke.Parent = Sidebar
local SidebarTop = Instance.new("Frame")
SidebarTop.Name = "SidebarTop" SidebarTop.Size = UDim2.new(1, 0, 1, -75) SidebarTop.BackgroundTransparency = 1 SidebarTop.BorderSizePixel = 0 SidebarTop.Parent = Sidebar
local Logo = Instance.new("TextLabel")
Logo.Name = "Logo" Logo.Size = UDim2.new(1, -24, 0, 40) Logo.Position = UDim2.new(0, 24, 0, 24) Logo.BackgroundTransparency = 1 Logo.Text = "BROSA SYSTEM" Logo.TextColor3 = Colors.TextMain Logo.Font = Enum.Font.SourceSansBold Logo.TextSize = 13 Logo.TextXAlignment = Enum.TextXAlignment.Left Logo.Parent = SidebarTop
local NavList = Instance.new("Frame")
NavList.Name = "NavList" NavList.Size = UDim2.new(1, -24, 1, -80) NavList.Position = UDim2.new(0, 12, 0, 80) NavList.BackgroundTransparency = 1 NavList.BorderSizePixel = 0 NavList.Parent = SidebarTop
local NavLayout = Instance.new("UIListLayout")
NavLayout.Parent = NavList NavLayout.SortOrder = Enum.SortOrder.LayoutOrder NavLayout.Padding = UDim.new(0, 4)
local PagesContainer = Instance.new("Frame")
PagesContainer.Name = "PagesContainer" PagesContainer.Size = UDim2.new(1, -230, 1, 0) PagesContainer.Position = UDim2.new(0, 230, 0, 0) PagesContainer.BackgroundColor3 = Colors.BgPanel PagesContainer.BorderSizePixel = 0 PagesContainer.Parent = MainPanel
local NavButtons = {}
...
for , pageFrame in pairs(PagesContainer:GetChildren()) do if pageFrame:IsA("ScrollingFrame") then pageFrame.Visible = (pageFrame.Name == "Page" .. pageName) end end
end
local function createNavItem(displayName, targetPage, layoutOrder)
local NavItem = Instance.new("TextButton")
NavItem.Name = "Nav_" .. targetPage NavItem.Size = UDim2.new(1, 0, 0, 38) NavItem.BackgroundTransparency = 0 NavItem.BackgroundColor3 = Color3.fromRGB(0, 0, 0) NavItem.BorderSizePixel = 0 NavItem.Text = " " .. displayName NavItem.TextColor3 = (layoutOrder == 1) and Colors.TextMain or Colors.TextMuted NavItem.Font = Enum.Font.SourceSansProSemibold NavItem.TextSize = 13 NavItem.TextXAlignment = Enum.TextXAlignment.Left NavItem.LayoutOrder = layoutOrder NavItem.Parent = NavList
local ButtonCorner = Instance.new("UICorner") ButtonCorner.CornerRadius = UDim.new(0, 6) ButtonCorner.Parent = NavItem
local ActiveIndicator = Instance.new("Frame")
ActiveIndicator.Name = "ActiveIndicator" ActiveIndicator.Size = UDim2.new(0, 2, 0, 19) ActiveIndicator.Position = UDim2.new(0, 0, 0.5, -9) ActiveIndicator.BackgroundColor3 = Colors.Accent ActiveIndicator.BorderSizePixel = 0 ActiveIndicator.BackgroundTransparency = (layoutOrder == 1) and 0 or 1 ActiveIndicator.Parent = NavItem
if layoutOrder == 1 then NavItem.BackgroundColor3 = Color3.fromRGB(8, 8, 26) end
NavButtons[targetPage] = NavItem
NavItem.MouseEnter:Connect(function() if _G.BrosaHub.Cache.ActivePage ~= targetPage then TweenService:Create(NavItem, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {TextColor3 = Colors.TextMain, BackgroundColor3 = Color3.fromRGB(6, 6, 6)}):Play() end end)
NavItem.MouseLeave:Connect(function() if _G.BrosaHub.Cache.ActivePage ~= targetPage then TweenService:Create(NavItem, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {TextColor3 = Colors.TextMuted, BackgroundColor3 = Color3.fromRGB(0, 0, 0)}):Play() end end)
...
ProfileBox.Name = "ProfileBox" ProfileBox.Size = UDim2.new(1, -24, 0, 56) ProfileBox.Position = UDim2.new(0, 12, 1, -68) ProfileBox.BackgroundColor3 = Colors.BgCard ProfileBox.BorderSizePixel = 0 ProfileBox.Parent = Sidebar
local ProfileCorner = Instance.new("UICorner") ProfileCorner.CornerRadius = UDim.new(0, 8) ProfileCorner.Parent = ProfileBox
local ProfileStroke = Instance.new("UIStroke") ProfileStroke.Color = Colors.Border ProfileStroke.Thickness = 1 ProfileStroke.Parent = ProfileBox
local AvatarMini = Instance.new("Frame")
AvatarMini.Name = "AvatarMini" AvatarMini.Size = UDim2.new(0, 32, 0, 32) AvatarMini.Position = UDim2.new(0, 12, 0.5, -16) AvatarMini.BorderSizePixel = 0 AvatarMini.Parent = ProfileBox
local AvatarCorner = Instance.new("UICorner") AvatarCorner.CornerRadius = UDim.new(1, 0) AvatarCorner.Parent = AvatarMini
local AvatarGradient = Instance.new("UIGradient")
AvatarGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Colors.Accent), ColorSequenceKeypoint.new(1, Color3.fromRGB(165, 180, 252))}) AvatarGradient.Rotation = 135 AvatarGradient.Parent = AvatarMini
local AvatarText = Instance.new("TextLabel")
AvatarText.Name = "AvatarText" AvatarText.Size = UDim2.new(1, 0, 1, 0) AvatarText.BackgroundTransparency = 1 AvatarText.Text = "BS" AvatarText.TextColor3 = Color3.fromRGB(255, 255, 255) AvatarText.Font = Enum.Font.SourceSansBold AvatarText.TextSize = 11 AvatarText.Parent = AvatarMini
local MetaMini = Instance.new("Frame")
MetaMini.Name = "MetaMini" MetaMini.Size = UDim2.new(1, -62, 1, 0) MetaMini.Position = UDim2.new(0, 52, 0, 0) MetaMini.BackgroundTransparency = 1 MetaMini.BorderSizePixel = 0 MetaMini.Parent = ProfileBox
local NameMini = Instance.new("TextLabel")
NameMini.Name = "NameMini" NameMini.Size = UDim2.new(1, 0, 0, 16) NameMini.Position = UDim2.new(0, 0, 0.5, -14) NameMini.BackgroundTransparency = 1 NameMini.Text = lp.Name or "Delta User" NameMini.TextColor3 = Colors.TextMain NameMini.Font = Enum.Font.SourceSansBold NameMini.TextSize = 12 NameMini.TextXAlignment = Enum.TextXAlignment.Left NameMini.Parent = MetaMini
local StatusMini = Instance.new("TextLabel")
StatusMini.Name = "StatusMini" StatusMini.Size = UDim2.new(1, 0, 0, 14) StatusMini.Position = UDim2.new(0, 0, 0.5, 2) StatusMini.BackgroundTransparency = 1 StatusMini.Text = "Active Premium" StatusMini.TextColor3 = Colors.StatusGreen StatusMini.Font = Enum.Font.SourceSansPro StatusMini.TextSize = 10 StatusMini.TextXAlignment = Enum.TextXAlignment.Left StatusMini.Parent = MetaMini

-- ============================================================================
-- 👑 BROSA SYSTEM v5.2 — PRIVATE ELITE MONOLITH SCRIPT HUB [ЧАСТЬ 2 ИЗ 3]
-- ============================================================================
...
local function createFeatureCard(parentPage, featureName, featureDesc)
	...
	Card.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement then TweenService:Create(Card, TweenInfo.new(0.25), {BackgroundColor3 = Color3.fromRGB(23, 23, 28)}):Play() TweenService:Create(CardStroke, TweenInfo.new(0.25), {Color = Colors.Accent}):Play() end end)
	Card.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement then TweenService:Create(Card, TweenInfo.new(0.25), {BackgroundColor3 = Colors.BgCard}):Play() TweenService:Create(CardStroke, TweenInfo.new(0.25), {Color = Colors.Border}):Play() end end)
	return Card
end
...
-- Инициализация структуры элементов страниц
...
if state then for _, p in pairs(Players:GetPlayers()) do if p ~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then _G.BrosaHub.Cache.OrbitTarget = p break end end else _G.BrosaHub.Cache.OrbitTarget = nil end
end)
...
if state then local char = getChar() local hum = getHum() if char and hum then local clone = hum:Clone() hum:Destroy() clone.Parent = char camera.CameraSubject = char:WaitForChild("Humanoid") end end
end)
...
if myRoot:FindFirstChild("BrosaFlyBV") then myRoot.BrosaFlyBV:Destroy() end if myRoot:FindFirstChild("BrosaFlyBG") then myRoot.BrosaFlyBG:Destroy() end
end
end)
...
if state then local players = Players:GetPlayers() if #players > 1 then local randomPlayer = players[math.random(1, #players)] while randomPlayer == lp do randomPlayer = players[math.random(1, #players)] end if randomPlayer.Character and randomPlayer.Character:FindFirstChild("HumanoidRootPart") then local myRoot = getRoot() if myRoot then myRoot.CFrame = randomPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 2, 0) end end end _G.BrosaHub.Flags.TPToPlayer = false end
end)
...
if state then Lighting.Brightness = 4 Lighting.GlobalShadows = false Lighting.Ambient = Color3.fromRGB(255, 255, 255) Lighting.OutdoorAmbient = Color3.fromRGB(255, 255, 255)
else Lighting.Brightness = 2 Lighting.GlobalShadows = true Lighting.Ambient = Color3.fromRGB(128, 128, 128) Lighting.OutdoorAmbient = Color3.fromRGB(128, 128, 128) end
end)
...
-- ============================================================================
-- 👑 BROSA SYSTEM v5.2 — PRIVATE ELITE MONOLITH SCRIPT HUB [ЧАСТЬ 3 ИЗ 3]
-- ============================================================================
...
if G.BrosaHub.Flags.TracerESP then
...
TweenService:Create(MainPanel, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 0}):Play()
else
TweenService:Create(MainPanel, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1}):Play()
task.wait(0.2)
...
print("[👑 BROSA HUB v5.2]: Монолит полностью скомпилирован, функции синхронизированы!")
